...
import { rescaleText } from './cityVisualisation';

const _filesData: IListFile[] = [];
...
export class GUI {
	...
	public constructor(bigBoard: BigBoard, container: HTMLElement, merger: Merger) {
		...
		conf = Object.assign(conf, {
			...
			// 'light color': '#' + bigBoard.light3.color.getHex().toString(16),
			// intensity: bigBoard.light3.intensity,
			// 'ambient color': '#' + bigBoard.ambient3.color.getHex().toString(16),
	...
	public async filesToInsert(list: IListFile[]): Promise<number> {
		...
		void Promise.all([
			...
			// 	sizeText.onChange(() => rescaleText(sizeText.getValue()));
	...
	private _initInteraction(container: HTMLElement): void {
		...
		// const bigBoard = this._bigBoard;
		// conf['light color'] = '#' + bigBoard.light3.color.getHex().toString(16);
		// conf.intensity = bigBoard.light3.intensity;
		// conf['ambient color'] = '#' + bigBoard.ambient3.color.getHex().toString(16);
		// conf.exportCountry = bigBoard.orthographic;
		// Light
		// const lightFolder = gui.addFolder('Light');
		// lightFolder.add(bigBoard.ambient3, 'intensity', 0, 5, 0.01).name('ambient intensity');
		// lightFolder.addColor(conf, 'light color').onChange((v: string) => {
		// 	const color = Number.parseInt(v.replace('#', ''), 16);
		// 	bigBoard.light3.color.setHex(color);
		// 	bigBoard.helper.color = color;
		// 	bigBoard.helper.update();
		// });
		// lightFolder.addColor(conf, 'ambient color').onChange((v: string) => {
		// 	const color = Number.parseInt(v.replace('#', ''), 16);
		// 	bigBoard.ambient3.color.setHex(color);
		// });
		// lightFolder
		// 	.add(conf, 'intensity', 0, 5, 0.01)
		// 	.name('light intensity')
		// 	.onChange((v: number) => {
		// 		bigBoard.light3.intensity = v;
		// 		bigBoard.helper.update();
		// 	});
		// lightFolder.add(bigBoard.light3.position, 'x', -100, 100, 1).onChange(() => bigBoard.helper.update());
		// lightFolder.add(bigBoard.light3.position, 'y', -100, 100, 1).onChange(() => bigBoard.helper.update());
		// lightFolder.add(bigBoard.light3.position, 'z', -100, 100, 1).onChange(() => bigBoard.helper.update());
		// lightFolder.add(bigBoard.light3.shadow.mapSize, 'width', 0, 1000).step(1);
		// lightFolder.add(bigBoard.light3.shadow.mapSize, 'height', 0, 1000).step(1);
		// lightFolder.add(bigBoard.light3.shadow.camera, 'near', 0, 1000).step(0.5);
		// lightFolder.add(bigBoard.light3.shadow.camera, 'far', 0, 1000).step(1);
		// Generalities
		...
		// 	// TODO why ?? bigBoard.getMergerI.merge();
		// 	bigBoard.addLegend();
		// });
		...
		// });
		// generalFolder.add(bigBoard, 'showCitiesName').name('Show Cities name');
		// cones
		...
		// // coneFolder.add(CONFIGURATION, 'intrudedHeightRatio', 0, 1).step(0.01);
