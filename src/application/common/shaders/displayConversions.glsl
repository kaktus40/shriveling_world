...
vec3 eckert(in vec3 pos, in float threeRadius, in float earthRadius, in vec3 reference, in float zzCoeff) {
  vec3 resultat = vec3(0.0);
  float theta = reference.y;
  for(int i = 0; i < ECKERT_ITERATION; i++) {
    theta += deltaEckert(theta, pos.y);
  }
  resultat.x = (pos.x - reference.x) * (1.0 + cos(theta)) / ECKERT_CONST * threeRadius;
  resultat.y = 2.0 * theta / ECKERT_CONST * threeRadius;
  resultat.z = (pos.z - reference.z) / earthRadius * threeRadius * zzCoeff;
  return resultat;
}

vec3 vanDerGrinten(in vec3 pos, in float threeRadius, in float earthRadius, in vec3 reference, in float zzCoeff) {
  vec3 resultat = vec3(0.0);
  resultat.z = (pos.z - reference.z) / earthRadius * threeRadius * zzCoeff;
...
vec3 conicEquidistant(in vec3 pos, in float threeRadius, in float earthRadius, in vec3 reference, in float standardParallel1, in float standardParallel2, in float zzCoeff) {
  vec3 resultat = vec3(0.0);
...
vec3 equirectangular(in vec3 pos, in float threeRadius, in float earthRadius, in vec3 reference, in float zzCoeff) {
  vec3 resultat = vec3(0.0);
...
vec3 mercator(in vec3 pos, in float threeRadius, in float earthRadius, in float lambda0, in float zzCoeff) {
  vec3 resultat = vec3(0.0);
...
vec3 winkel(in vec3 pos, in float threeRadius, in float earthRadius, in vec3 reference, in float zzCoeff) {
  vec3 resultat = vec3(0.0);
  ...
  resultat.y = ((pos.y - reference.y) + sinPhi / cardinalAlpha) * threeRadius / 2.0;
  resultat.z = (pos.z - reference.z) / earthRadius * threeRadius * zzCoeff;
  return resultat;
}

vec3 convertor(in vec3 pos, in float threeRadius, in float earthRadius, in vec3 reference, in float standardParallel1, in float standardParallel2, int representation, in int conesShape, in float zzCoeff) {
  vec3 resultat;
  if(representation == 0) {
    ...
    resultat = conicEquidistant(pos, threeRadius, earthRadius, reference, standardParallel1, standardParallel2, zzCoeff);
  }
  return resultat;
}

vec3 transit(in vec3 pos, in float threeRadius, in float earthRadius, in vec3 reference, in float standardParallel1, in float standardParallel2, in int projectionInit, in int projectionEnd, in float percent, in int conesShape, in float zzCoeff) {
  vec3 resultat;
  if(projectionInit == projectionEnd) {
    resultat = convertor(pos, threeRadius, earthRadius, reference, standardParallel1, standardParallel2, projectionInit, conesShape, zzCoeff);
  } else {
    vec3 initVec = convertor(pos, threeRadius, earthRadius, reference, standardParallel1, standardParallel2, projectionInit, conesShape, zzCoeff);
    vec3 endVec = convertor(pos, threeRadius, earthRadius, reference, standardParallel1, standardParallel2, projectionEnd, conesShape, zzCoeff);
